...
export class EnvironmentInspector {
  ...
  checkServiceAvailability(): {
    ...
    return {
      ...
      stripe: !!(this.validatedEnv.STRIPE_SECRET_KEY && this.validatedEnv.STRIPE_WEBHOOK_SECRET),
      email: !!(this.validatedEnv.SMTP_HOST && this.validatedEnv.SMTP_USER),
      crypto: !!(this.validatedEnv.BTC_ADDRESS && this.validatedEnv.ETH_ADDRESS),
  ...
  logInspection(): void {
    ...
    const missingCritical = criticalServices.filter(service => !services[service as keyof typeof services]);
